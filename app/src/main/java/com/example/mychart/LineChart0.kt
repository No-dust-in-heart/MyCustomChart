...
class LineChart0(mcontext: Context?, attributeSet: AttributeSet?, def: Int) : View(mcontext, attributeSet, def) {
    ...
    private fun drawYAxisAndXScaleValue(canvas: Canvas) {
            ...
            for (i in 0..(yLabel!!.size - 1)) {
                ...
                if (style != 1) {//右边刻度值
                    canvas.drawText(
                        ...
                        startPointX + bounds.width() - xScale / 15 + xLength,
                        startPointY + yScale * i + bounds.height() / 2,
                        mScaleValuePaint
                    )
                }
            ...
            when(style){
                1->{//获取坐标
                    ...
                    for (i in 0..(data!!.size - 1)) {
                        path.lineTo(
                            startPointX + xScale * i,
                            yLength - ((data!![i].toFloat() * yLength) / yLabel!![yLabel!!.size - 1]!!.toFloat()) + startPointY
                        )
                        flag = i
                    }
                    path.lineTo(startPointX + (xScale * flag), startPointY + yLength)
                    Log.d("LineChart0", flag.toString())
            ...
            /*for (i in 0..(data!!.size - 1)) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    canvas.drawRoundRect(
                        mDataCoords!![i][0], mDataCoords!![i][1],
                        mDataCoords!![i + 1][0], mDataCoords!![i + 1][1],
                        5f, 5f,
                        mDataLinePaint
                    )
                }
        ...
        private fun drawDataPoints(canvas: Canvas) {
            ...
            if (isClick && clickIndex > -1) {
                ...
                canvas.drawCircle(
                    mDataCoords!![clickIndex][0],
                    mDataCoords!![clickIndex][1],
                    xScale / 10,
                    mDataLinePaint
                )
                ...
                if (mDataCoords!![clickIndex][1] - xScale / 20 >= startPointY + yScale - 10) {
                    canvas.drawLine(
                        ...
                        startPointX + xScale * clickIndex, startPointY + yScale - 10,
                        mDataLinePaint
                    )
                } else {
                    canvas.drawLine(
        ...
        private fun initParams(width: Int, height: Int) {
            ...
            yScale = (height - 1.5f * ValueHeight) / (yLabel!!.size - 1) - 30         // y轴刻度单位
            if (style == 1) {
                xScale = (width - 2f * ValueWidth) / (xLabel!!.size - 1)      // x轴刻度单位
                xLength = (xLabel!!.size - 1) * xScale        // x轴长度
                startPointY = 0.5f * ValueHeight + 30   // 开始UI图的y坐标
            } else {
                xScale = (width - 2f * ValueWidth) / (xLabel!!.size + 1)
                xLength = (xLabel!!.size + 1) * xScale
                startPointY = 0.5f * ValueHeight + 80//给标题留出空间
            }
