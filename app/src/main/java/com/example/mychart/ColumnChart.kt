...
class ColumnChart(mcontext: Context?, attributeSet: AttributeSet?, def: Int) : View(mcontext, attributeSet, def) {
    ...
    init {
        ...
        if (xLabel == null && yLabel == null) {
            xLabel = arrayOf("10%", "50%", "100%")
            yLabel = arrayOf("0", "", "5", "10")
            yLabel2 = arrayOf("上座率", "5%", "15%", "30%")
        }
        // 数据值
        if (data == null && data2 == null) {
            data = arrayOf("2", "2.5", "3", "5","2", "2.5", "3", "5","10","8")
        }
        //数据坐标
    ...
    private fun initParams(width: Int, height: Int) {
        ...
        xScale = (width - 2 * ValueWidth) / 11      // x轴刻度单位
        startPointX = ValueWidth - 20       // 开始绘图的x坐标(左上角)
        startPointY = 2.5f * ValueHeight+40   // 开始UI图的y坐标
        xLength = (data!!.size + 1) * xScale        // x轴长度
        yLength = 3* yScale        // y轴长度

        val chartLineStrokeWidth = yScale / 50     // 坐标轴线条的线宽
        coordTextSize = yScale / 5             // 坐标刻度文字的大小
        val dataLineStrodeWidth = yScale / 50      // 数据线条的线宽
    ...
    private fun drawText(canvas: Canvas) {
      /*  var string = "总场次:${getSum(data!!)}"
        ...
        //标题文字
        mScaleValuePaint!!.getTextBounds("场次", 0, "场次".length, bounds)
        canvas.drawText("场次", startPointX-bounds.width()/2, startPointY-bounds.height()+5, mScaleValuePaint)

        val string3 = "上座率分布"
        mScaleValuePaint!!.textSize = 50f
        mScaleValuePaint!!.getTextBounds(string3, 0, string3.length, bounds)
        val x5 = startPointX - bounds.width() / 2 + 50
        val y5 = startPointY - 2f*bounds.height()
    ...
    * */
    private fun drawYAxisAndXScaleValue(canvas: Canvas) {
        ...
        for (i in 0..(xLabel!!.size - 1)) {
            ...
            if(i==0){
                ...
                canvas.drawText(
                    xLabel!![i],
                    startPointX + 5*i  * xScale - bounds.width() / 2,
                    startPointY + yLength + bounds.height() + yScale / 15,
                    mScaleValuePaint
                )
            }
    ...
    private fun drawXAxisAndYScaleValue(canvas: Canvas) {
        ...
        for (i in 0..(yLabel!!.size - 1)) {
            ...
            canvas.drawText(
                yLabel!![(yLabel!!.size - 1) - i],
                startPointX - bounds.width() - xScale / 15 -7,
            ...
            canvas.drawText(
                ...
                startPointX + xScale / 15 + xLength+5,
                startPointY + yScale * i + bounds.height() / 2+3,
                mScaleValuePaint
            )
    ...
    private fun showDetails(index: Int) {
        ...
        //偏移量
        val xoff = startPointX + xScale * (index + 1) - rect.width() / 2 -40
        val yoff = -(height + 0.75f * yScale).toInt() + 320
